...
from at30_strategy.strategy_base import Signal
...
class ExecutionEngine(LoggerMixin):
    ...
    async def execute(self, signal: Signal) -> Optional[dict[str, Any]]:
        ...
        ⚠️ **本方法是全系统唯一的下单出口, 但闸门由调用方负责。**
        交易闸门(`TradingGate`, 开仓/减仓两个方向的许可判定)**由调用方 `run.py` 在调用
        本方法之前完成**, 不在本方法内部 —— 也就是说这是「调用方记得判闸门」,
        而不是「出口本身无法绕过」。

        本层刻意**不持有**任何风险许可职权(见 `test_v142` 的结构性断言):
        执行器是「终端提交方」, 只负责把已批准的信号送出去。

        当前生产调用点只有 `run.py` 两处(均已过闸门), 由
        `tests/unit/test_v129_order_outlet_guard.py` 钉死: 一旦出现新调用点即变红。
        把闸门移进本方法可以根除这条路径, 但属架构改动, 见
        `docs/audits/trading-path-audit.md` §9(记为架构债, 不阻塞小资金验证)。
        """
