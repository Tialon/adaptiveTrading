...
1. **登记迁移**: 在本文「§3 迁移历史」追加一条, 说明新库自动创建 + 存量库需执行的手动
   `ALTER`(含建唯一索引时先去重)。
...
- 过滤 `sqlite_*` / `alembic_version` 内部表, 不参与判定。
- 该检查是**离线探测工具**, 未接入 `run.py` 启动路径(避免误报阻断启动); 建议纳入部署流程
  在升级后手动执行。

## 5. 存量库升级实操(SQLite → 默认 / MySQL → 生产)

1. **备份**(务必): `cp adaptive.db adaptive.db.bak`(或 MySQL `mysqldump`)。
2. 从「§3 迁移历史」确定当前已执行到的版本, 依次执行其后未执行的 `ALTER`。
3. 递增 `SCHEMA_VERSION` + 更新 `test_v129` 锚点(见 §2)。
4. 跑差异检查确认收敛:
   ```powershell
   .venv\Scripts\python -m at01_common.schema_check
   .venv\Scripts\python -m pytest tests/unit/test_v129_schema_audit.py tests/unit/test_v153_schema_check.py -q
   ```
5. 重启 `run.py`(重启自动 `create_all`, 补齐任何**新表**)。
