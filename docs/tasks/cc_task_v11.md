...
### P1-2 Optimizer V2 ✅(2026-09-08)

- `at70_backtest/backtest_optimizer.py`(新建): 网格搜索 → Walk-Forward → 鲁棒性 → 风险调整排序,
  防「历史最优 ≠ 未来最优」过拟合。
- `build_grid`(参数笛卡尔积展开)+ `OptimizerV2`(注入 evaluate 回调, 单点异常不阻断)。
- `build_param_result` 纯函数从 (train/test 收益序列) 派生: 均值/最坏/标准差/盈利占比/夏普/
  过拟合间隙(`compute_overfit_gap`= 训练均值-验证均值)/ 鲁棒性(复用 P1-1 `robustness_score`)。
- 风险调整得分 `= 鲁棒性 × max(0, 1+平均验证收益) × (1 - 0.5×clamp(过拟合间隙/10%))`:
  收益量级项打破鲁棒性的尺度不变, 过拟合间隙 10% → 得分减半, `rank_params` 按此降序赋 rank。
- 复用 P1-1: 从 `backtest_robustness` 抽出公共 `robustness_score(returns)` 供跨窗口鲁棒性复用。
- **回归测试 15 条**(`test_v117_optimizer_v2.py`): 网格展开 / 过拟合间隙 / 夏普 / 风险调整得分 /
